...
\begin{equation}\label{eq:ccacla-obsval}
...
Nodaļā tiks sniegts īss ieskats tālāk aprakstītajos eksperimentos~\ref{sec:eksp}
...
\begin{gather}
  \ddot{\theta} = \frac{g \sin\theta_t + \cos\theta_t (\frac{-F - m_p l \omega_t^2\sin\theta_t}
                                                         {m_c + m_p})}
                        {l (\frac{4}{3} - \frac{m_p\cos^2\theta_t}
                                               {m_c + m_p})} \\ 
  \ddot{x} = \frac{F + m_p l (\omega_t^2\sin\theta_t - \ddot{\theta}\cos\theta_t)}
                   {m_c + m_p} \\
  x_{t+1} = x_t + v_t \\
  v_{t+1} = v_t + \ddot{x} \\
...
\section{Veiktie eksperimenti un rezultāti}\label{sec:eksp}
...
algoritmu trenētajiem agēntiem tika pārbaudīts cart-pole uzdevumā. Katram no
algoritmiem tika veikti 10 eksperimenti. Atsevišķie eksperimenti tika apkopoti,
lai iegūtu attiecīgā algoritma vidējo rezultātu.
