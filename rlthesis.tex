...
\begin{equation}
  \hat{A}_\Theta:S \rightarrow A \times \mathbb{R},
\end{equation}
lai stāvoklī $s$ paredzētu gan optimālo darbību $a$, gan stāvokļa vērtību $v$ kā
$\hat{A}_\Theta(s) = (a, v)$. Funkcija $\hat{A}_\Theta$ tiek realizēta ar
...
varam arī izteikt funkciju $\hat{A}$
\begin{equation}\label{eq:ccacla-actval}
  \hat{A}(s) = (f_a(h^{(1 \ldots D(A))}), f_v(h^{(D(A) + 1)})), \text{ kur } 
...
darbības veikšanas. Tas noved pie izmaiņām funkcijā $\hat{A}$ 
\begin{equation}
  \hat{A}_\Theta':S \rightarrow A \times S \times \mathbb{R}
\end{equation}
...
Analogi vienādojumam \ref{eq:ccacla-actval} varam definēt funkciju $\hat{A}'$,
