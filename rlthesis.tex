...
algoritma, izmanto funkciju $\hat{A}_\Theta:S \rightarrow A \times \mathbb{R}$,
...
\begin{equation}
  a = V(s), V(s) = f_a(f_h(s^T \cdot W_h) \cdot W_a)
\end{equation} un
\begin{equation}
  v = Ac(s), Ac(s) = f_v(f_h(s^T \cdot W_h) \cdot W_v)
...
\begin{equation}\label{eq:ccacla-actval}
  \hat{A}(s) = (f_a(h^{(0 \ldots D(A))}), f_v(h^{(D(A) + 1)})), \text{ kur } 
